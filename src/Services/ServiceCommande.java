...
public class ServiceCommande implements IServiceCommande {
    ...
    public void create(Commande commande){
        try {
            String request
                    = "INSERT INTO commande (total_paiment,status,date,id_user) VALUES(?,?,?,?)";
    ...
    public boolean delete(int idCommande) throws SQLException {
        ...
        try {
            ...
            String req2 = "select id from product_cart where id_commande = ?";
            try {
    ...
    public boolean update(int idCommande) throws SQLException {
        String req = "update commande set status=1 where id=? ";
    ...
    public ObservableList<Commande> getAll() throws SQLException {
        ...
        try {
            ...
            ResultSet rs = pst.executeQuery();
            while (rs.next()){
                list.add(new Commande(rs.getInt("id"),rs.getFloat("total_paiment"),rs.getBoolean("status"),rs.getString("date"),rs.getInt("user_id")));
            }
        }catch (SQLException err){
            System.out.println(err.getMessage());
        }
    ...
    public List<Commande> readAll() throws SQLException {
       ...
       // String req = "select * from order o inner join user u on u.id=o.id_user";
        ...
        try {
            ...
            ResultSet rs = pst.executeQuery();
            while (rs.next()){
                list.add(new Commande(rs.getFloat("total_paiment"),rs.getBoolean("status"),rs.getString("date"),rs.getInt("user_id")));
                return list;
            }
        }catch (SQLException err){
            System.out.println(err.getMessage());
    ...
    public Commande getCommande(int idCommande) throws SQLException {
        ...
        try {
            ...
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                return new Commande(rs.getInt("id"), rs.getFloat("total_paiment"), rs.getBoolean("status"),rs.getString("date"), rs.getInt("user_id"));
            }
        } catch (SQLException err) {
            System.out.println(err.getMessage());
        }
    ...
    public ObservableList<PieChart.Data> getdata(){
        ...
        String request = "select count(status),date from commande where status = 0";
        try {
    ...
    public void historique(int idCommande, ObservableList<Cart> paniers) throws DocumentException, FileNotFoundException, BadElementException, SQLException {
        ...
        String file_name = "G:\\JobHub-Desktop-Application\\src\\Gui\\Commande\\Facture.pdf";
        ...
        Paragraph p1 = new Paragraph("Utilisateur: Mta ");
        //Paragraph p3 = new Paragraph("Email: " + user.getEmail());
        Paragraph p3 = new Paragraph("Email: mta@gmail.com" );
        Paragraph p2 = new Paragraph("Montant: " + this.getCommande(idCommande).getTotalPayment() + "$ ");
        try {
            document.add(Image.getInstance("G:\\JobHub-Desktop-Application\\images\\logo.png"));
        ...
        for (Cart panier : paniers) {
            try {
                table.addCell(Image.getInstance("G:JobHub-Desktop-Application\\src" + panier.getImage()));
            } catch (IOException ex) {
                Logger.getLogger(ServiceCommande.class.getName()).log(Level.SEVERE, null, ex);
            }
