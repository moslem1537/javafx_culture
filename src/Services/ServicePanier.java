...
public class ServicePanier implements IServicePanier {
    ...
    public void add(Panier p) throws SQLException {
        String req = "insert into product_cart (quantite,id_commande,id_produit) values (?,?,?)";
    ...
    public boolean update(Panier p) throws SQLException {
        String req = "update product_cart set quantite=? where id=?";
        try {
            PreparedStatement pst = cnx.prepareStatement(req);
            pst.setInt(1, p.getQuantity());
    ...
    public List<Panier> readAll() throws SQLException {
        ...
        try {
            ...
            while (rs.next()) {
               Panier p = new Panier(rs.getInt("quantite"),rs.getInt("id_commande"),rs.getInt("id_produit"));
                list.add(p);
            }
    ...
    public ObservableList<Panier> getPanier(int idC) {
        ...
        String requete = "select * from product_cart where id_commande=?";
        try{
            ...
            while (resultSet.next()){
                Panier panier = new Panier(resultSet.getInt("id"),resultSet.getInt("quantite"),
